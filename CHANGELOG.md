...
- The sync only ever asked AniList for the *current* calendar season, so a season's
  ...
  For fall 2026 that meant Oct 5 — days after the premieres, with the whole season
  missing from the site until then
- Added a lookahead: a run now covers the current season plus the upcoming one once it
  starts within `LOOKAHEAD_DAYS` (30). September runs already pull `fall-<year>`. Steps 1
  and 2 loop over the resolved seasons; a failure on one season does not affect the other
- `ALWAYS_INCLUDE_ANIME` (One Piece) is re-tagged with the current season only — a
  lookahead season has not started and must not claim those long-running entries
- The season is now derived from the month **in JST**. The cron fires at 21:00 UTC, which
  is already the next day in Japan, so the run on the last Sunday of a quarter used to ask
  for the season that had just ended
- Added `--season=fall-2026[,winter-2027]` to force the seasons of a manual catch-up run
- Entries pulled ahead have a future `startDate`. The home page already filtered those out;
  the weekly schedule grid did not, and would have announced a day and time for shows that
  had not premiered. Filtered in `schedule-grid.tsx` on the client, not on the server: the
  schedule page is static and only regenerated by the weekly sync, so a server-side date
  comparison would freeze and could keep a show hidden for a week after its premiere

### fix: An AniList outage no longer costs the whole sync run
- Runs on 2026-08-02, 08-23 and 09-06 died on `AniList error: 403`. The body reads "The
  AniList API has been temporarily disabled due to severe stability issues" — an
  AniList-side outage, not the Cloudflare bot challenge that `b106074` addressed
- Backoff raised from 2/4/6/8s (gave up after 20s) to 15/45/90/180s, enough to ride out a
  short outage
- Added a circuit breaker: steps 2b and 3 call AniList once per anime, so once a call has
  exhausted its retries against a server-side failure every later call short-circuits
  instead of replaying a known outage across hundreds of anime. 429 does not trip it —
  that is our own request rate. Both loops now log the abort and stop early
- Each step in `main()` is isolated: a failure is collected into `errors[]` and the run
  continues, so an AniList outage still leaves step 2 (uzurea), step 4 (R2) and step 5
  (DeepL) to do their work. The process still exits 1, keeping the run red
- Failures now log AniList's own error message instead of a bare status code
- `sync-anime.yml`: the "Revalidate site cache" step runs with `if: !cancelled()`. Now that
  a partial run writes to the DB and exits 1, skipping revalidation would have left the
  static site showing stale data until the following Sunday
