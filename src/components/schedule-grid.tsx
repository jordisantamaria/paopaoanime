...
/**
 * The sync pulls the upcoming season up to 30 days before it starts, so the data
 * contains anime that have not premiered yet. The weekly grid announces a day and
 * time as if the show were on the air, so it must only list what already premiered.
 *
 * This has to happen on the client: the schedule page is statically generated and
 * only regenerated by the weekly sync, so a server-side date comparison would be
 * frozen at build time and could keep a show hidden for a week after its premiere.
 * Same reasoning as the "latest anime" list in `home-content.tsx`.
 */
function usePremieredByDay(
  animeByDay: Record<DayOfWeek, AnimeEntry[]>
): Record<DayOfWeek, AnimeEntry[]> {
  return useMemo(() => {
    const now = new Date();
    const hasPremiered = (a: AnimeEntry) =>
      !a.startDate || new Date(a.startDate + "T00:00:00+09:00") <= now;
    return Object.fromEntries(
      DAYS.map((day) => [day, (animeByDay[day] ?? []).filter(hasPremiered)])
    ) as Record<DayOfWeek, AnimeEntry[]>;
  }, [animeByDay]);
}

export function ScheduleGrid({ animeByDay: allAnimeByDay, nonWeeklyAnime = [] }: Props) {
  const animeByDay = usePremieredByDay(allAnimeByDay);
